...
var init_params = __esm({
  "core/params.js"() {
    ParamSystem = class {
       ...
       * Skips aliases (multiple IDs pointing to the same node)
...
var __filename, __dirname, loadParams, R9DS_PARAMS, JB202_PARAMS, JB01_PARAMS, JT30_PARAMS, JT10_PARAMS, JT90_PARAMS, SYNTH_PARAMS;
...
var init_jb202_node = __esm({
  "instruments/jb202-node.js"() {
    ...
    JB202Node = class extends InstrumentNode {
      ...
      /**
       * Serialize JB202 state (sparse format)
...
var init_jb01_node = __esm({
  "instruments/jb01-node.js"() {
    ...
    JB01Node = class extends InstrumentNode {
      ...
      /**
       * Serialize JB01 state (sparse format)
      ...
      serialize() {
        ...
        for (const [path, value] of Object.entries(this._params)) {
          ...
          const paramDef = JB01_PARAMS[voice]?.[paramName];
          if (paramDef) {
            const defaultEngine = paramDef.unit === "semitones" || paramDef.unit === "choice" ? paramDef.default : toEngine(paramDef.default, paramDef);
            if (Math.abs(value - defaultEngine) > 1e-3) {
              sparseParams[path] = value;
            }
          } else if (path === "level" && value !== -6) {
            sparseParams[path] = value;
          }
        }
        return {
          ...
          patternLength: this._pattern.kick?.length || 16,
          params: Object.keys(sparseParams).length > 0 ? sparseParams : void 0
        };
      }
      /**
      ...
      deserialize(data) {
        if (data.pattern) {
          ...
          if (isSparse) {
            this._pattern = createEmptyPattern2(length);
        ...
        if (data.params) {
          for (const [path, value] of Object.entries(data.params)) {
            ...
            if (paramDef && typeof value === "number") {
              if (paramDef.unit === "0-100" && value > 1.5) {
                this._params[path] = toEngine(value, paramDef);
  ...
  "../web/public/jt90/dist/machines/jt90/engine.js"() {
    ...
    JT90Engine = class _JT90Engine {
      ...
      async renderPattern(options = {}) {
        ...
        const stepsPerBar = 16;
        const totalSteps = bars * stepsPerBar;
        const baseStepDuration = stepDuration ?? 60 / renderBpm / 4;
        ...
        for (let step = 0; step < totalSteps; step++) {
          ...
          const stepDuration2 = renderSwing > 0 ? baseStepDuration * (step % 2 === 1 ? 1 + swingFactor : 1 - swingFactor) : baseStepDuration;
          const stepSamples = Math.floor(stepDuration2 * sampleRate);
          for (let i = 0; i < stepSamples && sampleIndex < totalSamples; i++, sampleIndex++) {
            let sample = 0;
            for (const voiceId of VOICE_IDS2) {
...
function generateJB202Midi(session, outputPath) {
  const bars = session.bars || 2;
  const ppq = 96;
  const pattern = session.jb202Pattern || session.bassPattern || [];
  const trackEvents = [
...
function generateFullMidi(session, outputPath) {
  ...
  const jb202Track = [
    ...trackNameEvent("JB202 Bass"),
    ...melodicPatternToMidi(session.jb202Pattern || session.bassPattern || [], 0, bars, ppq)
  ];
  const midiData = [
    ...generateHeader(1, 3, ppq),
    // Format 1, 3 tracks (tempo + 2 instruments)
...
__export(session_tools_exports, {
  ...
  createEmptyJB202Pattern: () => createEmptyJB202Pattern,
...
function showJB202(session) {
  const node = session._nodes.jb202;
  const params = node._params;
  const defs = JB202_PARAMS.bass;
  const pattern = node.getPattern();
  const lines = ["JB202 MODULAR BASS SYNTH", ""];
  lines.push("OSC1: " + [
...
function createEmptyJB202Pattern() {
  return Array(16).fill(null).map(() => ({
    note: "C2",
    gate: false,
    accent: false,
...
var JB202_BASS_PARAMS, createEmptyJB200Pattern, sessionTools;
var init_session_tools = __esm({
  "tools/session-tools.js"() {
    init_tools();
    init_converters();
    JB202_BASS_PARAMS = JB202_PARAMS.bass || {};
    createEmptyJB200Pattern = createEmptyJB202Pattern;
    sessionTools = {
      /**
      ...
      create_session: async (input, session, context) => {
        ...
        session.jb202Pattern = createEmptyJB202Pattern();
        session.jb202Params = {
          osc1Waveform: "sawtooth",
          osc1Octave: 0,
          osc1Detune: 0.5,
       ...
       * Generic tool that works with all synths: jb01, jb202, sampler
       */
      show: async (input, session, context) => {
        ...
        if (!showFn) {
          const available = ["jb01", "jb202", "sampler"];
...
var JT90_VOICES, jtTools;
var init_jt_tools = __esm({
  "tools/jt-tools.js"() {
    init_tools();
    init_converters();
    JT90_VOICES = ["kick", "snare", "clap", "rimshot", "lowtom", "midtom", "hitom", "ch", "oh", "crash", "ride"];
    jtTools = {
      // ==================== JT10 (Lead Synth) ====================
      /**
      ...
      tweak_jt30: async (input, session, context) => {
        ...
        if (input.mute === true) {
          ...
          session._nodes?.jt30?.setParam("bass.level", 1);
          tweaks.push("unmuted");
        }
        const paramMap = {
          level: "level",
      ...
      tweak_jt90: async (input, session, context) => {
        ...
        if (input.mute === true) {
          ...
          session._nodes?.jt90?.setParam(`${voice}.level`, 1);
          tweaks.push("unmuted");
        }
        if (input.level !== void 0) {
    ...
    songTools = {
      ...
      load_pattern: async (input, session, context) => {
        ...
        if (instrument === "jt10") {
          ...
          session.jt10Params = JSON.parse(JSON.stringify(saved.params));
        ...
        if (instrument === "jt30") {
          ...
          session.jt30Params = JSON.parse(JSON.stringify(saved.params));
        ...
        if (instrument === "jt90") {
          ...
          session.jt90Params = JSON.parse(JSON.stringify(saved.params));
      ...
      list_patterns: async (input, session, context) => {
        const lines = [];
        for (const instrument of ["jb01", "jb202", "sampler", "jt10", "jt30", "jt90"]) {
          const patterns = session.patterns?.[instrument] || {};
...
async function renderSession(session, bars, filename) {
  ...
  const canonicalIds = ["jb01", "jb202", "jp9000", "sampler", "jt10", "jt30", "jt90"];
